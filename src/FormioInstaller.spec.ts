...
describe("FormioImporter", () => {
  ...
  describe("createRootUser()", () => {
    it("should create the user root", async () => {
      const { service, formioService } = await createFormioInstallerFixture({
        count: 1,
      });
      const template = {
        resources: {
      ...
      expect(
        formioService.resources.submission.model.create
    ...
    it("should throw error", async () => {
      const { service } = await createFormioInstallerFixture({
        errorSubmission: new Error("message"),
      });
      const template = {
        resources: {
          admin: {
